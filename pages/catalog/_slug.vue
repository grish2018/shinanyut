<template>
  <div class="wrapper">
    <a class="backLink" href="#" @click.prevent="goBack">
      <Icon class="header-menu__icon header-menu__icon--angle-left" name="gm-angle-right" /> <span>{{ this.$route.query.categoryName }}</span>
    </a>
    <ul class="header-menu">
      <li class="header-menu__item" v-for="(item,index) in subCategory" :key="index">
        <a class="header-menu__link" href="#" @click.prevent="toSubCategory(item.slug,item.name)">
          {{ item.name }}
...
export default {
  computed: {
    subCategory() {
      ...
      const items = categories.find((item) => `${item.slug}-${item.id}` === this.$route.params.slug);
  ...
  methods: {
    toSubCategory(slug, name) {
      const { category } = this.$route.query;
      this.$router.push({
        path: '/catalog/product',
        query: { category, slug, name },
      });
    },
    goBack() {
      this.$router.back();
